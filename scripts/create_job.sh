...
# There will be one job script per leaf directory, and it may contain repeated execution
# of the same combination of machine, rank, app and args.
      ...
      while IFS= read -r args || [[ -n "$args" ]]; do
        ...
        for i_r in ${repeats}; do
          ...
          if [ "${scheduler[$m]}" == "flux" ] ; then
            ...
            echo 'ofile=`echo '${outfile}' | sed -e '"'"'s/\.txt/-'"'"'${FLUX_ENCLOSING_ID}'"'"'\.txt/'"'"'`' >>  ${jobfile}
          else
            if [ "${bank}" != "" ] ; then
            ...
            echo '' >>  ${jobfile}
            echo 'ofile=`echo '${outfile}' | sed -e '"'"'s/\.txt/-'"'"'${SLURM_JOBID}'"'"'\.txt/'"'"'`' >>  ${jobfile}
          fi
          ...
          if [ "${hpcrun_cmd}" != "" ] ; then
            ...
            for evt in "${hpcrun_events[@]}"; do
              ...
              echo "     ${hpcrun_cmd} -o hpctoolkit-measurements-evt$((i_evt++)) ${evt}"'\' >>  ${jobfile}
