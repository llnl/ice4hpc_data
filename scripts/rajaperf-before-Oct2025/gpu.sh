...
machines="corona ec2-p3 ec2-p4d lassen pascal tioga"
...
gpu_type["ec2-p3"]=Base_CUDA
gpu_type["ec2-p4d"]=Base_CUDA
gpu_type["lassen"]=Base_CUDA
gpu_type["pascal"]=Base_CUDA
        ...
        if [ ${print_header} -eq 1 ] ; then
          ...
          awk -v m=${m} -v Type=${gpu_type[${m}]} \
            'BEGIN{FS="[[:space:]]*,[[:space:]]*"; OFS=","}
             (NR==1) { print Type ; for (i=1; i<=NF; i++) { if ($i ~ Type) { col = i; break; } } }
