  ...
  def __call__(self, config_yaml, target_app, preprocessor1, A_X_train, A_Y_train, A_tar_x_scaled, A_tar_y_scaled, rank):
    ...
    for nu_of_frozen_layers in use_case_specific_config["num_of_frozen_layers"]:
      for i in range(rank, rank+1):
        ...
        os.makedirs(os.path.dirname(f"{csv_path}{folder_name}/Source-model-on-target-{target_app}-q-c-{folder_name}-results-{rank}-MSE.csv"), exist_ok=True)
        os.makedirs(os.path.dirname(f"{csv_path}{folder_name}/Source-model-on-target-{target_app}-q-c-{folder_name}-results-{rank}-MAPE.csv"), exist_ok=True)
        fileI = open(f"{csv_path}{folder_name}/Source-model-on-target-{target_app}-q-c-{folder_name}-results-{rank}-MSE.csv", "w")
        fileJ = open(f"{csv_path}{folder_name}/Source-model-on-target-{target_app}-q-c-{folder_name}-results-{rank}-MAPE.csv", "w")
        writerI = csv.writer(fileI)
        writerJ = csv.writer(fileJ)
        tar_x_train, tar_x_test, tar_y_train, tar_y_test = preprocessor2.get_train_test_target(test_size = 0.9, rand_state=i*50)
        ech = 5
        for j in test_samples:
          #n = j/10.0
          rowMSE = []
          ...
          if isinstance(j, int):
            ...
            fname = f"{j}-percent"
            fidx = int(j*10.0)
          tar_x_scaled, tar_y_scaled = preprocessor2.get_tar_train()
          print("before the sample loader")
          x2, lb2, tar_x_scaled, tar_y_scaled = util.sampleLoader(tar_x_scaled, tar_y_scaled,f"{indices_path}/{target_app}-q-c-indices-{rank}-{fname}.csv" ,j)
          print(f"The indices path is {indices_path}/{target_app}-indices-{rank}-{fname}.csv")
          print("After the sample loader")
          
          source_model = util.sorceModelLoader( B_X_train, B_Y_train, self.transfer_permission, nu_of_frozen_layers, False, global_config, target_app, False,os.getcwd()+global_config["source_model2"], os.getcwd()+global_config["source_model_weights2"] )
          mse0, mse2, mape0, mape2 = util.update_source_model(source_model, tar_x_test, tar_y_test, x2, lb2, global_config, ech)
          
          source_model = util.sorceModelLoader( B_X_train, B_Y_train, False, nu_of_frozen_layers, False, global_config, target_app, False,os.getcwd()+global_config["source_model2"], os.getcwd()+global_config["source_model_weights2"])
          predictions = source_model.predict(tar_x_test)
          mse0 = mean_squared_error(tar_y_test, predictions)

          ground_truth = preprocessor2.transform(tar_y_test)
          model_predictions = preprocessor2.transform(predictions)
          mape0 = mean_absolute_percentage_error(ground_truth, model_predictions)
          ...
          ground_truth = preprocessor2.transform(tar_y_test)
          model_predictions = preprocessor2.transform(pp1)
