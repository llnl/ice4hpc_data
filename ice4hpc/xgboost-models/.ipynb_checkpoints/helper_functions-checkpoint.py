...
def get_data() -> pd.DataFrame:
    ...
    data_df = data_df.drop(columns=['duration','PM_MATH_FLOP_CMPL','L1-DCACHE-LOAD-MISSES','L1-ICACHE-LOAD-MISSES'])
    data_df = data_df.dropna(subset=['REALTIME (sec)'])
    return data_df

def show_data_info(data_df):
    temp_df = data_df.drop(columns=['machine','app','args','ranks','REALTIME (sec)'])
    num_rows, num_cols = temp_df.shape
    print(f"Number of samples: {num_rows}, Number of features: {num_cols}")
    print("Total number of -1: {}".format((temp_df == -1).sum().sum()))
    numeric_df = temp_df.select_dtypes(include='number')
    print("Total number of negatives: {}".format((numeric_df < 0).sum().sum()))
    print("Total number of 0: {}".format((temp_df == 0).sum().sum()))
    print("Total number of NA: {}".format(temp_df.isna().sum().sum()))
    non_numeric_mask = temp_df.applymap(lambda x: not pd.api.types.is_number(x))
    print("Total number of non-numeric: {}".format(non_numeric_mask.sum().sum()))

def normalize(data_df) -> pd.DataFrame:
    # replace '-1', which encodes N/A, with '0'
    cols_to_replace = ['L1_STM', 'L2_LDM', 'L2_STM', 'FP_SINGLE', 'FP_DOUBLE', 'ARITH']
    data_df[cols_to_replace] = data_df[cols_to_replace].replace(-1, 0)

    cols_to_divide = ['BR_INS','LD_INS','SR_INS','TOT_INS','L1_LDM',
                      'L1_STM','L2_LDM','L2_STM','FP_SINGLE',
                      'FP_DOUBLE','ARITH','IO Bytes Read',
                      'IO Bytes Written','MEM_WCY','Overhead']
    data_df[cols_to_divide] = data_df[cols_to_divide].div(data_df['REALTIME (sec)'], axis=0)
    return data_df
...
def remove_unneeded_columns(data_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Already remove duration
    refined_df = data_df.drop(columns=['app', 'args', 'target machine', 'source machine', 'Overhead', 'sid', 'target_sid'])
