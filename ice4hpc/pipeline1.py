...
def show_feature_info(data_df: pd.DataFrame, f: TextIO = None):
    ...
    non_numeric_mask = temp_df.map(lambda x: not pd.api.types.is_number(x))
...
def use_tabnet(data_df: pd.DataFrame,
    ...
    try:
        ...
        # Extract embeddings
        embeddings = pretrainer.transform(X_float, "concat")
...
def feat_transform(X_train_raw: pd.DataFrame,
    ...
    if X_test_raw is not None:
        ...
        if use_embedding :
            if (f_feat is not None) and (pretrainer is not None):
...
def main():
    ...
    kfold_rounds = make_KFold_rounds(X_train_all_raw, y_train_all, gpu_q, use_embedding)
